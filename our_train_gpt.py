...
class Hyperparameters:
    ...
    jepa_enabled = bool(int(os.environ.get("JEPA_ENABLED", "0")))
...
class LatentProjector(nn.Module):
    def __init__(self, in_dim: int, latent_dim: int):
        super().__init__()
        self.norm = RMSNorm()
        self.fc1 = CastedLinear(in_dim, latent_dim, bias=False)
        self.fc2 = CastedLinear(latent_dim, latent_dim, bias=False)
    def forward(self, x: Tensor) -> Tensor:
        x = self.norm(x)
...
class GPT(nn.Module):
    def __init__(
        ...
        if self.jepa_enabled:
            ...
            self.jepa_span_embed = nn.Embedding(len(self.jepa_future_spans), jepa_latent_dim)
            # Target encoder doesn't need gradients — updated via EMA
    ...
    def _compute_jepa_loss(self, mid_hidden: Tensor, target_hidden: Tensor) -> Tensor:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.jepa_enabled:
            main_loss = main_loss + self.jepa_loss_weight * self._compute_jepa_loss(mid_hidden, x.reshape(input_ids.shape[0], input_ids.shape[1], -1))
...
def main() -> None:
    ...
    # JEPA trainable params go to scalar optimizer (target encoder excluded — EMA only)
    if args.jepa_enabled and base_model.jepa_context_encoder is not None:
        for module in [base_model.jepa_context_encoder, base_model.jepa_predictor]:
            for p in module.parameters():
                scalar_params.append(p)
        if base_model.jepa_span_embed is not None:
            scalar_params.append(base_model.jepa_span_embed.weight)
    ...
    while True:
        ...
        with torch.no_grad():
            ...
            if args.jepa_enabled and base_model.jepa_context_encoder is not None:
                for tp, sp in zip(base_model.jepa_target_encoder.parameters(),
                                   base_model.jepa_context_encoder.parameters()):
                    tp.data.mul_(args.jepa_ema_decay).add_(sp.data, alpha=1.0 - args.jepa_ema_decay)
        step += 1
    ...
    export_sd = {k: v for k, v in full_state_dict.items() if "mtp_heads" not in k and "jepa_" not in k}
    excluded_mtp = sum(int(t.numel()) for k, t in full_state_dict.items() if "mtp_heads" in k)
    excluded_jepa = sum(int(t.numel()) for k, t in full_state_dict.items() if "jepa_" in k)
    if excluded_jepa > 0:
